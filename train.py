...
#
def git_projectname(path):

        git_repo = git.Repo(path, search_parent_directories=True)
        return git_repo.working_dir.split('/').pop()
#
# parse command-line arguments
#
parser = argparse.ArgumentParser(description='PyTorch ImageNet Training')

parser.add_argument('--project-name', type=str, default=git_projectname(os.path.basename(__file__)))
...
def main_worker(gpu, ngpus_per_node, args):
    global best_acc1
    args.gpu = gpu
    run_name = f"{args.arch}__{args.project_name}__{args.seed}__{int(time.time())}"
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # writer.add_scalar('charts/learning_rate', args.lr, epoch)
